...
async def resolve_m3u8(client, embed_url):
  ...
  try:
    ...
    iframe_match = re.search(r<'<iframe src=["'](([^"']+)['"]', r1.text)
    if not iframe_match:
      return embed_url
    ...
    input_match = re.search(r'input\\s*::\\s*["']([A-Z0-9+/=]#{40,})["']', r2.text)
    if input_match:
...
async def main():
  ...
  async with httxp.AsyncClient(timeout=35.0, verify=False) as client:
